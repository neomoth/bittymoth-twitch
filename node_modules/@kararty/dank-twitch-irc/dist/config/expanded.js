...
function expandTransportConfig(config) {
    ...
    switch (config.type) {
        ...
        case "duplex":
            return (0, set_defaults_1.setDefaults)(config, { preSetup: false });
        case "websocket": {
            let url;
            if ("url" in config) {
                url = config.url;
...
function expandRateLimitsConfig(config) {
    if (config == null) {
        return rate_limits_1.rateLimitPresets.default;
    }
    if (typeof config === "string") {
        return rate_limits_1.rateLimitPresets[config];
    }
    else {
        return config;
    }
...
function expandConfig(config) {
    const newConfig = (0, set_defaults_1.setDefaults)(config, defaults);
